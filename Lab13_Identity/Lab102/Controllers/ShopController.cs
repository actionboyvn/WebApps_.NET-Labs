...
namespace Lab102.Controllers
{
    public class ShopController : Controller
    {
        ...
        public IActionResult Order()
        {
            ...
            ViewBag.paymentMethods = new SelectList(new List<String> { "Online transfer", "BLIK", "Cash on delivery" });
            return View(order);
        }
        [HttpPost]
        [Authorize]
        public IActionResult OrderConfirmation([Bind("Products,Quantities,Cost,Name,Phone,Address,PaymentMethod")] OrderViewModel order)
        {
            Dictionary<int, int> quantities = getQuantities();
            var context = _context.Articles.
                Include(a => a.Category).
                Where(a => quantities.Keys.Contains(a.Id));
            order.Products = context;
            order.Quantities = quantities;
            order.Cost = calcTotalCost();
            if (ModelState.IsValid)
            {                
                saveQuantities(new Dictionary<int, int>());
                return View(order);
            }
